...
from reflector.db.jobs import JobStatus, jobs
...
def parse_s3_url(url: str) -> Optional[Tuple[str, str]]:
    ...
    if "s3.amazonaws.com" in url or re.search(r"s3[.-][\w-]+\.amazonaws\.com", url):
        ...
        if parsed.hostname:
            ...
            if ".s3.amazonaws.com" in parsed.hostname or re.match(
                r".*\.s3[.-][\w-]+\.amazonaws\.com", parsed.hostname
            ):
                bucket = parsed.hostname.split(".s3")[0]
                key = parsed.path.lstrip("/")
                return bucket, key
            # Path-style
            elif parsed.hostname == "s3.amazonaws.com" or re.match(
                r"s3[.-][\w-]+\.amazonaws\.com", parsed.hostname
            ):
...
async def download_audio_file(audio_url: str) -> str:
    ...
    try:
        ...
        if (
            ...
            or "s3.amazonaws.com" in audio_url
            or re.search(r"s3[.-][\w-]+\.amazonaws\.com", audio_url)
        ):
    ...
    async def _process():
        ...
        try:
            ...
            progress_reporter = create_progress_reporter(
                job_id, update_job_status, JobStatus.PROCESSING
            )
            ...
            logger.info(
                f"Job {job_id} completed successfully with {len(events)} events"
            )
