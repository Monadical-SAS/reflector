...
class OpenAILLM(LLM):
    ...
    def __init__(
        ...
        **overrides,
        ...
        self.config = (
            self._load_config(config_prefix, overrides) if config_prefix else {}
        )

        # Use provided model_name or fall back to config
        self.model_name = (
            model_name or self.config.get("model") or self.config.get("llm_model")
        )

        # Set up HTTP client
        self.client = httpx.AsyncClient(
            timeout=self.config.get("llm_timeout", 300), headers=self._get_headers()
        )
    ...
    def _set_defaults(self, config: Dict[str, Any]):
        ...
        if "llm_url" not in config:
            config["llm_url"] = getattr(
                settings, "LLM_URL", "https://api.openai.com/v1"
            )
    ...
    async def _generate(
        self, prompt: str, gen_schema=None, gen_cfg=None, logger=None, **kwargs
    ) -> str:
        """
