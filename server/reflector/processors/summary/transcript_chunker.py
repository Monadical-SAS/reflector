...
async def process_transcript_with_template_aware_chunking(
    ...
    llm_completion_func: Callable[
        ..., Awaitable[str]
    ],  # str being json - accepts (messages, **kwargs)
    create_prompt_func: Callable[[str], str],
    validate_json_func: Callable,
    create_dedup_prompt_func: Optional[
        Callable[[str], str]
    ] = None,  # Function for deduplication prompt
    ...
    In reality, the case of >1 chunk is a rare case. Even a tech full hour tech talk could fit into 160k token limit - currently a rather small token limit of a self-hosted model.
    ...
    if template_overhead_tokens >= max_context_tokens:
        raise ValueError(
            f"Template overhead ({template_overhead_tokens}) exceeds context limit ({max_context_tokens})"
        )

    # Calculate available space for transcript content
    max_content_tokens = (
        max_context_tokens - template_overhead_tokens - safety_margin_tokens
    )
    ...
    logger.debug(
        f"Chunking: max_content={max_content_tokens}, core={core_content_tokens}, overlap={overlap_tokens}"
    )
    ...
    for i, chunk in enumerate(chunks):
        ...
        if actual_tokens > max_context_tokens:
            logger.warning(
                f"Chunk {i} too large ({actual_tokens} tokens), shrinking..."
            )

            chunk = _shrink_chunk_to_fit(
                chunk, messages_template, create_prompt_func, max_context_tokens, logger
            )
...
def _generate_chunks_with_overlap(
    ...
    while current_pos < len(transcript) and chunk_count < max_chunks:
        ...
        chunk_end = find_natural_split_point(
            transcript, chunk_end, current_pos + core_chars
        )
...
async def _process_chunk(
    ...
    if not isinstance(subjects, list):
        raise ValueError(
            f"Invalid subjects: {subjects}, validate_json_func must return a list of subjects or raise an exception"
        )
    return subjects
...
async def _deduplicate_subjects(
    ...
    messages.messages = [
        msg for msg in messages.messages if msg["role"] != "user"
    ]  # Remove any existing user messages
    ...
    if isinstance(consolidated_subjects, list):
        logger.info(
            f"Consolidated {len(all_subjects)} subjects into {len(consolidated_subjects)}"
        )
        return consolidated_subjects
    else:
        logger.warning(
            "LLM returned non-list result, falling back to basic deduplication"
        )
