...
class TestTemplateAwareChunking:
    ...
    async def test_single_chunk_no_splitting(self):
        ...
        messages_template = MockMessages(
            [{"role": "system", "content": "You are an assistant."}], tokenizer
        )
    ...
    async def test_template_overhead_too_large(self):
        ...
        messages_template = MockMessages(
            [{"role": "system", "content": "Very long system prompt" * 100}], tokenizer
        )
        ...
        with pytest.raises(
            ValueError, match="Template overhead .* exceeds context limit"
        ):
    ...
    async def test_empty_transcript(self):
        ...
        messages_template = MockMessages(
            [{"role": "system", "content": "Assistant"}], tokenizer
        )
