...
def upgrade() -> None:
    ...
    op.drop_table('_alembic_tmp_meeting')
    ...
    with op.batch_alter_table('transcript', schema=None) as batch_op:
        ...
        batch_op.create_index('idx_transcript_recording_id', ['recording_id'], unique=False)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('recording', schema=None) as batch_op:
        ...
        batch_op.create_unique_constraint('uq_recording_object_key', ['bucket_name', 'object_key'])

    with op.batch_alter_table('meeting', schema=None) as batch_op:
        batch_op.drop_index('idx_meeting_room_id')

    op.create_table('_alembic_tmp_meeting',
    ...
    sa.Column('room_mode', sa.VARCHAR(), server_default=sa.text("'normal'"), nullable=False),
    sa.Column('recording_type', sa.VARCHAR(), server_default=sa.text("'cloud'"), nullable=False),
    sa.Column('recording_trigger', sa.VARCHAR(), server_default=sa.text("'automatic-2nd-participant'"), nullable=False),
    sa.Column('num_clients', sa.INTEGER(), server_default=sa.text('0'), nullable=False),
