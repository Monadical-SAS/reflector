# Source: https://docs.daily.co/reference/rest-api/rooms/live-streaming/start
...
Try Daily pay-as-you-go features for free! New accounts have a $15 credit automatically applied when you add a credit card to the account. Learn more about pay-as-you-go features on our [pricing page](https://www.daily.co/pricing).

Starts a live stream at the `rtmpUrl` specified.

Multiple RTMP URLs can be passed using the `endpoints` property, which accepts an array of RTMP URLs. (See [`addLiveStreamingEndpoints()`](/reference/daily-js/instance-methods/add-live-streaming-endpoints) for a code example using this property.)
...
[Contact us](https://www.daily.co/contact/support) to configure `max_streaming_instances_per_room` for your domain. Note: recording and streaming instances that share the same `instanceId` also share the same underlying composition process, so any layout change made to streaming will also affect recording, and vice-versa.

A `503` status code may be returned if there are not enough workers available. In this case, you may retry the request again.

If you would like to use both RTMP and HLS, we recommend that you define [`streaming_endpoints`](/reference/rest-api/rooms/config#streaming_endpoints) for both types of endpoints.
...
[Next](/reference/rest-api/rooms/live-streaming/update)
