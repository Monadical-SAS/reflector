...
class TestS3URLParsing:
    ...
    def test_s3_protocol_url(self):
        ...
        assert parse_s3_url("s3://my-bucket/path/to/file.mp4") == (
            "my-bucket",
            "path/to/file.mp4",
        )
    ...
    def test_virtual_hosted_style_url(self):
        ...
        assert parse_s3_url("https://my-bucket.s3.amazonaws.com/path/to/file.mp4") == (
            "my-bucket",
            "path/to/file.mp4",
        )
        assert parse_s3_url("https://bucket.s3.us-west-2.amazonaws.com/file.wav") == (
            "bucket",
            "file.wav",
        )
    ...
    def test_path_style_url(self):
        ...
        assert parse_s3_url("https://s3.amazonaws.com/my-bucket/path/to/file.mp4") == (
            "my-bucket",
            "path/to/file.mp4",
        )
        assert parse_s3_url("https://s3-us-west-2.amazonaws.com/bucket/file.wav") == (
            "bucket",
            "file.wav",
        )
...
class TestAudioFileDownload:
    ...
    async def test_download_s3_url(self, mock_tempfile, mock_s3_download):
        ...
        mock_s3_download.assert_called_once_with(
            "s3://my-bucket/audio/test.mp4", "/tmp/test_abc123.mp4"
        )
    ...
    @patch("reflector.worker.audio_tasks.tempfile.NamedTemporaryFile")
    async def test_download_http_url(
        self, mock_tempfile, mock_http_download, mock_validate
    ):
        ...
        mock_http_download.assert_called_once_with(
            "https://example.com/audio/test.mp4", "/tmp/test_xyz789.mp4"
        )
    ...
    @patch("reflector.worker.audio_tasks.tempfile.NamedTemporaryFile")
    async def test_download_storage_key(
        self, mock_tempfile, mock_settings, mock_s3_download
    ):
        ...
        mock_s3_download.assert_called_once_with(
            "s3://reflector-media/audio/uploads/test.mp4", "/tmp/test_key123.mp4"
        )
...
class TestS3Download:
    ...
    async def test_s3_download_success(self, mock_file, mock_session):
        ...
        mock_s3_client.download_fileobj.assert_called_once_with(
            "my-bucket", "path/to/file.mp4", mock_file()
        )
