...
def mock_celery_diarization_task():
    ...
    with patch(
        "reflector.views.audio.process_audio_with_diarization_task"
    ) as mock_task:
...
class TestAudioDiarizeEndpoint:
    ...
    async def test_diarize_audio_success(
        ...
        assert (
            call_args.kwargs["args"][4] is False
        )  # only_transcript (always False for diarization)
        assert call_args.kwargs["args"][5] == "modal"  # diarization_backend
        assert call_args.kwargs["time_limit"] == 900  # 900000ms / 1000

    @pytest.mark.asyncio
    async def test_diarize_audio_default_options(
        self, client: TestClient, mock_celery_diarization_task
    ):
        """Test audio diarization with default options"""
        request_data = {"audio_url": "https://example.com/audio.wav"}
    ...
    async def test_diarize_audio_invalid_url(self, client: TestClient):
        ...
        request_data = {"audio_url": "not-a-valid-url"}
    ...
    async def test_get_job_results_success(self, client: TestClient, db_session):
        ...
        result_data = [
            ...
            },
            {
                ...
                "uid": "def456",
                "data": {"translation": {"text": "Hola mundo", "language": "es"}},
            },
        ]
