...
def main():
    ...
    if url.scheme == "http" or url.scheme == "https":
        ...
        if media_file.endswith(".m4a"):
            subprocess.run(
                ["ffmpeg", "-i", media_file, f"./artefacts/{media_file}.mp4"]
            )
    ...
    with open(
        "./artefacts/transcript_" + NOW.strftime("%m-%d-%Y_%H:%M:%S") + ".txt", "w"
    ) as transcript_file:
