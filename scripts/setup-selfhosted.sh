...
step_caddyfile() {
    ...
    if [[ -n "$PRIMARY_IP" ]]; then
        # On a droplet/server: explicit IP so Caddy provisions cert at startup
        cat > "$caddyfile" << CADDYEOF
# Generated by setup-selfhosted.sh — explicit IP for server
